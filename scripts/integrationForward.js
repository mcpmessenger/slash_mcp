...
(async () => {
  ...
  await Promise.all([
    sendRpc(wsA, {
      ...
      id: 1,
      method: 'mcp_register',
      params: { connectionId: connA },
    }),
    sendRpc(wsB, {
      jsonrpc: '2.0',
      id: 2,
      method: 'mcp_register',
      params: { connectionId: connB },
    }),
  ]);
  ...
  const forwardResp = await sendRpc(wsA, {
    jsonrpc: '2.0',
    id: outerId,
    method: 'mcp_forward',
    params: {
      targetConnectionId: connB,
      request: {
        jsonrpc: '2.0',
        id: innerId,
        method: 'mcp_invokeTool',
        params: { toolName: 'shell_execute', parameters: { command: pingCmd } },
      },
    },
  });
  if (forwardResp.error) {
    console.error('Forward failed', forwardResp.error);
    process.exit(1);
  }
  ...
  const listener = (data) => {
    try {
      ...
      if (msg.id === innerId && msg.result && msg.result.execId) {
        execId = msg.result.execId;
      }
      if (msg.method === 'mcp_streamOutput' && msg.params?.execId === execId) {
        streamChunks++;
      }
      if (msg.method === 'mcp_execComplete' && msg.params?.execId === execId) {
        execStatus = msg.params.status;
      }
    } catch {}
  };
  ...
  const sendRes = await sendRpc(wsB, {
    jsonrpc: '2.0',
    id: 200,
    method: 'mcp_sendResource',
    params: { type: 'text', content: textContent },
  });
  const resId = sendRes.result?.resourceId;
  let fetchOk = false;
  if (resId) {
    const fetch = await sendRpc(wsA, {
      jsonrpc: '2.0',
      id: 201,
  ...
  console.table([
    { test: 'forward & stream', pass: forwardPass, chunks: streamChunks, status: execStatus },
    { test: 'resource roundtrip', pass: fetchOk },
  ]);
