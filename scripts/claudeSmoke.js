...
async function run() {
  ...
  try {
    ...
    ok = res.ok &&
         json?.result?.stdout?.trim() === "hi" &&
         json?.result?.exitCode === 0;
  } catch (_) {
    // invalid JSON
    ok = false;
  }
  ...
  if (!ok) {
    if (res.status === 404 && /Session not found/i.test(text)) {
      console.error("Claude MCP smoke test FAILED – proxy requires a valid mcp-session-id. Try restarting it with --stateless or ensure initialize returns a session id.");
    } else {
