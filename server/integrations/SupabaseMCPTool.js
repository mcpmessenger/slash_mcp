...
registry.register({
  ...
  description:
    'Complete Supabase integration providing database, storage, auth, and real-time functionality',
  ...
  handler: async (params) => {
    ...
    try {
      ...
      const clientKey =
        operation.includes('user') || operation.includes('bucket') || operation === 'create_table'
      ...
      switch (operation) {
        ...
        case 'select':
          ...
          let selectQuery = supabase
            ...
            .select(
              operationParams.columns
                ? Array.isArray(operationParams.columns)
                  ? operationParams.columns.join(',')
                  : operationParams.columns
                : '*',
            );
          ...
          if (operationParams.offset) {
            selectQuery = selectQuery.range(
              ...
              operationParams.offset + (operationParams.limit || 100) - 1,
            );
          }
        ...
        case 'insert':
          if (!table || !operationParams.data) {
            throw {
              code: -32602,
              message: 'table and data parameters required for insert operation',
            };
          }
        ...
        case 'update':
          if (!table || !operationParams.data) {
            throw {
              code: -32602,
              message: 'table and data parameters required for update operation',
            };
          }
        ...
        case 'upsert':
          if (!table || !operationParams.data) {
            throw {
              code: -32602,
              message: 'table and data parameters required for upsert operation',
            };
          }
        ...
        case 'rpc':
          ...
          result = await supabase.rpc(
            operationParams.function_name,
            operationParams.function_args || {},
          );
          break;
        ...
        case 'upload_file':
          if (!operationParams.bucket || !operationParams.file_path || !operationParams.file_data) {
            throw {
              code: -32602,
              message:
                'bucket, file_path, and file_data parameters required for upload_file operation',
            };
          }
        ...
        case 'download_file':
          if (!operationParams.bucket || !operationParams.file_path) {
            throw {
              code: -32602,
              message: 'bucket and file_path parameters required for download_file operation',
            };
          }
          result = await supabase.storage
            .from(operationParams.bucket)
        ...
        case 'delete_file':
          if (!operationParams.bucket || !operationParams.file_path) {
            throw {
              code: -32602,
              message: 'bucket and file_path parameters required for delete_file operation',
            };
          }
          result = await supabase.storage
            .from(operationParams.bucket)
        ...
        case 'create_bucket':
          if (!operationParams.bucket) {
            throw {
              code: -32602,
              message: 'bucket parameter required for create_bucket operation',
            };
          }
        ...
        case 'delete_bucket':
          if (!operationParams.bucket) {
            throw {
              code: -32602,
              message: 'bucket parameter required for delete_bucket operation',
            };
          }
        ...
        case 'get_public_url':
          if (!operationParams.bucket || !operationParams.file_path) {
            throw {
              code: -32602,
              message: 'bucket and file_path parameters required for get_public_url operation',
            };
          }
        ...
        case 'sign_up':
          if (!operationParams.email || !operationParams.password) {
            throw {
              code: -32602,
              message: 'email and password parameters required for sign_up operation',
            };
          }
        ...
        case 'sign_in':
          if (!operationParams.email || !operationParams.password) {
            throw {
              code: -32602,
              message: 'email and password parameters required for sign_in operation',
            };
          }
