...
export const TerminalPane: React.FC<{ initialConnId: string }> = ({ initialConnId }) => {
  ...
  const suggestions = [...connections.slice(0, 2).map((c, idx) => `@${idx + 1} ls`), ...whitelist];
  ...
  const execute = async (cmd: string) => {
    ...
    if (verb.startsWith('@')) {
      ...
      if (!targetConn) {
        ...
        window.dispatchEvent(
          new CustomEvent('mcp_mirror', {
            detail: { targetConnId: targetConn.id, command: restParts.join(' ') },
          }),
        );
    ...
    if (verb === 'tool') {
      ...
      try {
        ...
        setEntries((prev) =>
          prev.map((e) =>
            e.id === id ? { ...e, output: err.message ?? 'Error', status: 'error' } : e,
          ),
        );
      }
      setHistory((prev) => [...prev, cmd]);
    ...
    try {
      ...
      if ('result' in res && res.result?.execId) {
        ...
        setEntries((prev) =>
          prev.map((e) =>
            e.id === id ? { ...e, output: res.error?.message ?? 'Error', status: 'error' } : e,
          ),
        );
      }
    } catch (err: any) {
      ...
      if (fallback) {
        setEntries((prev) =>
          prev.map((e) =>
            e.id === id ? { ...e, output: fallback.output, status: fallback.status } : e,
          ),
        );
      } else {
        setEntries((prev) =>
  ...
  useEffect(() => {
    const handler = (e: any) => {
      ...
      if (targetConnId === connId) {
        ...
        setEntries((prev) => [
          ...prev,
          { id, command, output: '(executed remotely)', status: 'success' },
        ]);
      }
    };
  ...
  const getLocalCommandOutput = (
    cmd: string,
  ): { output: string; status: 'success' | 'error' } | null => {
  ...
  return (
    <div className="flex flex-col h-full w-full border-r border-gray-700">
      ...
      <form
        ...
        className="p-2 bg-gray-900 border-b border-gray-700 flex items-center space-x-2"
      >
      ...
      <div
        ref={containerRef}
        className="flex-1 overflow-y-auto p-3 font-mono text-sm space-y-2 bg-gray-900 dark:bg-gray-950/80 text-gray-200"
      >
        {entries.map((en) => (
          <div key={en.id}>
            ...
            {en.status === 'running' ? (
              <div className="ml-4 flex items-center space-x-2">
                ...
                {en.execId && (
                  <button
                    onClick={() => invokeTool(connId, 'abort_exec', { execId: en.execId })}
                    title="Abort"
              ...
              <pre
                className={`ml-4 whitespace-pre-wrap break-words ${en.status === 'error' ? 'text-red-400' : 'text-gray-300'}`}
              >
                {en.output}
              </pre>
            )}
