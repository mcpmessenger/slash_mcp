...
export const OpposingTerminals: React.FC<OpposingTerminalsProps> = ({ onClose }) => {
  ...
  if (connections.length < 2) {
    return (
      <AnimatePresence>
        ...
        >
          <motion.div
            className="bg-gray-900 text-gray-100 p-6 rounded shadow-lg"
            initial={{ scale: 0.8 }}
            animate={{ scale: 1 }}
            exit={{ scale: 0.8 }}
          >
            <p>Connect at least two clients to use opposing terminals.</p>
            <button onClick={onClose} className="mt-4 px-3 py-1 bg-primary-600 rounded">
              Close
            </button>
          </motion.div>
        </motion.div>
  ...
  return (
    <AnimatePresence>
      ...
      >
        <motion.div
          className="relative w-full max-w-6xl h-[80vh] bg-gray-900 text-gray-100 rounded-lg shadow-lg flex flex-col"
          initial={{ scale: 0.8 }}
          animate={{ scale: 1 }}
          exit={{ scale: 0.8 }}
